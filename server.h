...
int GetUser(int sock, char * user, char * pass){
	...
	int ack = Verification(user, pass);
	SendAck(sock, ack);

	msg = (ack)?"Login Successful":"Wrong credentials";
...
int ShowBankingMenu(int sock, char * user){
	...
	if(ack==1)
		...
		if(!ack)
			response = "Wrong input. You will be logged out..";
		else
			response = "Thanks for using internet banking.";
